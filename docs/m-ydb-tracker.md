...
| M8 | conformance green local+docker+remote | ◐ | `m-driver-conformance` built (in m-driver-sdk). m-ydb static checks (caps+version) green; full local/docker run needs a real engine (yours). **Finding (open):** see CFM below. |
| CFM | **conformance finding: doctor envelope/exit mismatch** | ☐ | `m-driver-conformance --driver dist/m-ydb --transport local` → 15/16: `meta doctor` (unreachable) writes a stdout envelope `exit:0` but the process exits 6. Root cause is shared **clikit**: `cc.Result` always emits `{ok:true,exit:0}` to stdout; `Fail` writes the error envelope to *stderr*. So any "data + non-zero exit" verb (doctor-unreachable, and by the same pattern `sync status`/`exec`-fail drift) mismatches. Fix = a clikit `Result`-with-exit path applied byte-identically across m-ydb/m-iris/m-cli (coordinated clikit change — deferred). `meta version` is already conformant (no change needed). |
