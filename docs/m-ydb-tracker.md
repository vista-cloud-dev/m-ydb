...
| DGB3 | **source-store (load/sync) path now container-aware — CLOSED** | ☑ | DGB2 fixed the *exec* path; the **source axis** (`exec load`, `sync`) took a separate path — `config.SourceStore` resolved routine write-dirs from `ParseRoutinesDirs($ydb_routines)` off the **host** env only, never the container. Against vehu (no host `ydb_routines`) it returned `no routine source directory`, so `exec load` staged nothing — which is exactly how **`v pkg install` surfaced as `stage ZVPKGRD: driver loaded no routine`** (the OPEN P4 blocker in v-rpc-tap's reaper-live-proof; NOT a v-pkg bug, NOT a T0a.3 regression — that proof passed `M_YDB_ROUTINES` explicitly). **Fixed (2026-06-30):** new `Session.ContainerRoutines(ctx)` reads the engine's own `$ZROUTINES` (docker-only; no `--routines` → no override → login-shell value; `object*(src…)` form parses); `SourceStore` uses it as a fallback **only when host Routines empty + docker + container set** (strictly additive — explicit `--routines` + bare m-test-engine untouched, no regression). TDD `TestContainerRoutines_DockerReadsZRoutines`/`_NonDockerIsNoop`; `gofmt`/`vet`/`golangci-lint`/`go test`/`make test-it` green. **Live (zero flags):** `exec load` → loaded+compiled on vehu AND m-test-engine (was BAD_CONFIG). **Full `v pkg` lifecycle proven byte-clean on vehu:** install → **status 3 / installed:true** (3 greenfield routines); verify → all present; uninstall → byte-clean (source + `.o` + #9.7 entry + `^XTMP("VPKGI")` gone). ZVPKGRD P4 blocker fully CLOSED. (Transient `stage-incomplete: 794 of 345` was stale `^XTMP("VPKGI")` residue, NOT a bug — `vslrtap.kids` = exactly 345 pairs; clean stage = 345/345.) See memory [[m-ydb-docker-gbldir]]. |
